...
class ScanClass:        
    ...
    def CountColoni(self,labelsImg,roiandgray,roi):
        ...
        for label in np.unique(labelsImg):
            ...
            bacteriaColonies.append(print(bacteriaObject))
            
            FinalImage = cv2.putText(roi,str(countBacteria), centerOfBacteria, cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0,0,255), 1)
            CountColonies = cv2.drawContours(FinalImage,[biggestContourInAzone],-1,(255,0,0),1)

        return FinalImage, showed
